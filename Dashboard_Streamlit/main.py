...
# Convert 'Order Date' column to datetime if it's not already in the correct format
...
# Display filtered data without the index column
st.subheader('Filtered Data')
st.dataframe(filtered_data)

# Visualizations with updated width for the charts
st.subheader('Sales Data Visualizations')

# Sales vs. Profit using Plotly with updated plot styling
fig = px.scatter(filtered_data, x='Sales', y='Profit', color='Category', hover_data=['Product Name'])
fig.update_traces(marker=dict(size=8, line=dict(width=1, color='DarkSlateGrey')))
fig.update_layout(plot_bgcolor='white')
st.plotly_chart(fig, use_container_width=True)

# Total Sales by Ship Mode using Plotly with updated plot styling and different colors for each bar
...
# Total Profit by Category using Plotly with updated plot styling and different colors for each bar
